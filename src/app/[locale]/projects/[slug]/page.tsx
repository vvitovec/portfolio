...
const hasStructuredCaseStudyContent = (blocks: CaseStudyBlock[]) =>
  blocks.some((block) => {
    ...
    if (block.type === 'outcome') {
      return hasText(block.body) || block.bullets?.some((item) => item.trim().length > 0);
    }
    return false;
  });
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = project.descriptionShort ?? project.tagline ?? t('meta.fallbackDescription');
  ...
  const emptyCaseStudy = (
    <SectionReveal className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">{t('caseStudySummary')}</p>
      </div>
      <Card>
        <CardContent className="space-y-4 p-6 md:p-8">
          <p className="text-muted-foreground text-sm">{t('caseStudyEmpty')}</p>
