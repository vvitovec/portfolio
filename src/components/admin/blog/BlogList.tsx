...
export default function BlogList() {
  ...
  const rows = useMemo(() => {
    ...
    return data.filter((post) =>
      [getTitle(post), post.slug, ...post.tags].join(' ').toLowerCase().includes(query),
    );
  }, [data, search]);
            ...
            <TableBody>
              {rows.map((post) => (
                <BlogRow
                  ...
                  onSendNewsletter={() => sendNewsletterMutation.mutate({ postId: post.id })}
    ...
    <TableRow>
      <TableCell>
        <div className="space-y-1">
          <p className="text-foreground font-medium">{getTitle(post)}</p>
          <p className="text-muted-foreground max-w-[28rem] truncate text-xs">/blog/{post.slug}</p>
        </div>
      </TableCell>
      <TableCell>
            ...
            wasSentToNewsletter ? (
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="outline" size="sm" disabled={isSendingNewsletter}>
                    {t('actions.sendNewsletter')}
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>{t('confirmNewsletter.title')}</AlertDialogTitle>
                    <AlertDialogDescription>
                      {t('confirmNewsletter.description')}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>{t('confirmNewsletter.cancel')}</AlertDialogCancel>
